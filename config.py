# Configuration for Project Samarth Enhanced
DATA_GOV_API_KEY = "579b464db66ec23bdd000001b99df47d66174b7771f8db787f44753c"
CROP_RESOURCE_ID = "35be999b-0208-4354-b557-f6ca9a5355de"
RAINFALL_RESOURCE_ID = "6c05cd1b-ed59-40c2-bc31-e314f39c6971"

# Local fallback CSV paths (relative). Keep your original CSV files here or adjust paths.
LOCAL_CROP_CSV = "crop_data_long.csv"
LOCAL_RAINFALL_CSV = "rainfall_data.csv"

# Caching directory (will be created automatically)
CACHE_DIR = "cache"
